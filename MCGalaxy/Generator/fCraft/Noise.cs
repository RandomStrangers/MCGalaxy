...
namespace MCGalaxy.Generator.fCraft
{
    ...
    public sealed class Noise
    {
        ...
        public static float InterpolateCosine(float v0, float v1, float x) => (float)(v0 * (1 - ((1 - Math.Cos(x * Math.PI)) * .5)) + v1 * ((1 - Math.Cos(x * Math.PI)) * .5));
        ...
        public float InterpolatedNoise(float x, float y)
        {
            ...
            float yFloat = y - yInt, p00, p01, p10, p11;
        ...
        {
            float total = 0,
                frequency = (float)Math.Pow(2, startOctave),
                amplitude = (float)Math.Pow(decay, startOctave);
        ...
        public static float[] GaussianBlur5X5(float[] heightmap, int width, int length)
        {
            ...
            for (int x = 0, i = 0; x < width; x++)
                ...
                {
                    output[i] = x < 2 || y < 2 || x > width - 3 || y > length - 3
                        ? heightmap[i]
