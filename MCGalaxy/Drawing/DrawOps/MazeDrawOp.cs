...
namespace MCGalaxy.Drawing.Ops
{
    public class MazeDrawOp : CuboidHollowsDrawOp
    {
        ...
        void GenerateMaze()
        {
            wall = new bool[width + 1, length + 1];
            ...
            Stack<GridNode> stack = new(width * length);
            ...
            while (true)
            {
                ...
                switch (dir)
                {
                    case 0:
                        if (IsWall(P.X, P.Y + 2))
                        {
                            P1 = new(P.X, (ushort)(P.Y + 1));
                            P2 = new(P.X, (ushort)(P.Y + 2));
                            return;
                        }
                        break;
                    case 1:
                    ...
                    case 2:
                        if (IsWall(P.X + 2, P.Y))
                        {
                            P1 = new((ushort)(P.X + 1), P.Y);
                            P2 = new((ushort)(P.X + 2), P.Y);
                            return;
                        }
                        break;
                    case 3:
        ...
        bool IsWall(int x, int y) => x >= 0 && y >= 0 && x <= width && y <= length && wall[x, y];
        struct GridNode
        {
            public ushort X, Y;
