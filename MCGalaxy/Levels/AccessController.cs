...
namespace MCGalaxy
{
    ...
    public abstract class AccessController
    {
        ...
        public bool CheckAllowed(Player p) => Check(p.name, p.Rank) == AccessResult.Accepted || Check(p.name, p.Rank) == AccessResult.Whitelisted;
        public AccessResult Check(string name, LevelPermission rank) => Blacklisted.CaselessContains(name)
                ? AccessResult.Blacklisted
                : Whitelisted.CaselessContains(name)
                ? AccessResult.Whitelisted
                : rank < Min
        ...
        /// </summary>
        public bool Describe(Player p, StringBuilder perms, bool shorten = false)
        {
