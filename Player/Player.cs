    ...
    public sealed partial class Player : IDisposable {
        ...
        void HandleLogin(byte[] message)
        {
            try
            {
                ...
                isGCMod = Server.GCmods.CaselessContains(truename);
        ...
        void CompleteLoginProcess() {
            ...
            if (Server.Owner != "" && Server.Owner.ToLower().Equals(name.ToLower())) {
        ...
        public void SetPrefix() { 
            string viptitle = isDev ? string.Format("{1}[{0}Dev{1}] ", c.Parse("maroon"), color) : isMod ? string.Format("{1}[{0}Mod{1}] ", c.Parse("lime"), color) : isGCMod ? string.Format("{1}[{0}GCMod{1}] ", c.Parse("gold"), color) : "";
