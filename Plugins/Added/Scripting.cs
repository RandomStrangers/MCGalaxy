...
namespace MCGalaxy.Scripting2
{
    ...
    public static class IScripting
    {
        ...
        static byte[] GetDebugData(string path)
        {
            ...
            {
                Server.s.Log("Error loading .pdb " + path + " : " + ex);
                return null;
            }
        }
        public static List<T> LoadTypes<T>(Assembly lib)
        {
            ...
            foreach (Type t in lib.GetTypes())
            {
                ...
                if (instance == null)
                {
                    Server.s.Log(typeof(T).Name + "\"" + t.Name + "\" could not be loaded");
        ...
        static string DescribeLoadError(string path, Exception ex)
        {
            ...
            Server.s.Log("Error loading commands from " + path + " : " + ex);
        ...
        public static bool LoadPlugin(string path, bool auto)
        {
            try
            {
                ...
                List<Plugin2> plugins2 = IScripting.LoadTypes<Plugin2>(lib);

                foreach (Plugin2 plugin2 in plugins2)
                {
                    if (!Plugin2.Load(plugin2, auto)) return false;
                }
                return true;
            }
            catch (Exception ex)
            {
                Server.s.Log("Error loading plugins from " + path + " : " + ex);
                return false;
            }
        }
    }
    ...
    public abstract class ICompiler
    {
        ...
        {
            return FormatSource(PluginSkeleton, plugin, creator, Server.Version1);
        }
    ...
    public abstract class ICodeDomCompiler : ICompiler
    {
        ...
        void InitCompiler()
        {
            lock (compilerLock)
            {
                ...
                Server.s.Log("WARNING: " + FullName + " compiler is missing, you will be unable to compile files.");
            }
        }
    ...
    class SourceMap
    {
        ...
        public string Get(string file, int line)
        {
            ...
            if (source == null)
            {
                try
                {
                    source = Utils3.ReadAllLinesList(file);
                }
                catch
                {
