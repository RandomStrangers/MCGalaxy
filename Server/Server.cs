...
namespace MCGalaxy
{
    public sealed class Server
    {
        ...
        {
            if (ClassicubeAccountPlus == false)
                return name.RemoveLastPlus2();
            else if (ClassicubeAccountPlus == true)
            return name;
            return name;
        }
        public static bool ClassicubeAccountPlus = true;

        public static string SourceURL = "https://github.com/RandomStrangers/MCGalaxy/";
            ...
            Log("Starting Server");
            {
                ...
                if (!File.Exists("MySql.Data.dll"))
                {
                    ...
                    try
                    {
                        ...
                        {
                            WEB.DownloadFile("https://github.com/RandomStrangers/MCGalaxy/blob/master/Uploads/MySql.Data.dll?raw=true", "MySql.Data.dll");
                        }
                        if (File.Exists("MySql.Data.dll"))
                        {
                ...
                if (!File.Exists("System.Data.SQLite.dll"))
                {
                    ...
                    try
                    {
                        using (WebClient WEB = new WebClient())
                        {
                            WEB.DownloadFile("https://github.com/RandomStrangers/MCGalaxy/blob/master/Uploads/System.Data.SQLite.dll?raw=true", "System.Data.SQLite.dll");
                        }
                        if (File.Exists("System.Data.SQLite.dll"))
                        {
                ...
                if (!File.Exists("sqlite3.dll"))
                {
                    ...
                    try
                    {
                        using (WebClient WEB = new WebClient())
                        {
                            WEB.DownloadFile("https://github.com/RandomStrangers/MCGalaxy/blob/master/Uploads/sqlite3.dll?raw=true", "sqlite3.dll");
                        }
                        if (File.Exists("sqlite3.dll"))
                        {
                ...
                if (!File.Exists("Newtonsoft.Json.dll"))
                {
                	...
                	try
                	{
                		using (WebClient WEB = new WebClient())
                		{
                            WEB.DownloadFile("https://github.com/RandomStrangers/MCGalaxy/blob/master/Uploads/Newtonsoft.Json.dll?raw=true", "Newtonsoft.Json.dll");
                		}
                		if (File.Exists("Newtonsoft.Json.dll"))
                		{
	  ...
	  UseTextures = true;          
        ...
        public void UpdateStaffList() {
            try {
                ...
                gcmods.Clear();
                using (WebClient web = new WebClient()) {
                    string[] result = web.DownloadString("https://github.com/RandomStrangers/MCGalaxy/raw/master/Uploads/devs.txt").Split(new string[] { Environment.NewLine, "\n" }, StringSplitOptions.None);
                ...
                ErrorLog(e);
                s.Log("Couldn't update MCGalaxy staff list, using defaults. . .");
                devs.Clear();
                mods.Clear();
                gcmods.Clear();
                devs.Add("hetal728");
