...
mcgdir="./"							##  Folder containing MCGalaxy.exe
...
gamepid="${mcgdir}/mcg.pid"			##  If you do not know what this is, do not worry about it, for "silent" mode only.
gamelog="${mcgdir}/mcg.log"			##  This logs everything sent to console, if started in "silent" mode
...
if [ ! -f "${mcgdir}/MCGalaxy.exe" ]; then
	echo -e "cannot find ${mcgdir}/MCGalaxy.exe!"
...
case "$1" in
	silent)
		...
		if ps -ef |grep "${monobin} ${gameopt} ${mcgdir}/MCGalaxy.exe"|awk -F" " -v game=${monobin} 'BEGIN {status=1} ; $8 == game {status=0} ; END {exit status}' ; then
			...
			if ${monobin} ${gameopt} ${mcgdir}/MCGalaxy.exe 1>> ${gamelog} 2>&1 & sleep 3 ; then
				pid=`ps -ef |grep "${monobin} ${gameopt} ${mcgdir}/MCGalaxy.exe"|awk -F" " -v game=${monobin} 'BEGIN {status=1} ; $8 == game { print $2 } ; END {exit status}'`
				echo ${pid} > ${gamepid}
...
;;
	normal)
		...
		if ps -ef |grep "${monobin} ${gameopt} ${mcgdir}/MCGalaxy.exe"|awk -F" " -v game=${monobin} 'BEGIN {status=1} ; $8 == game {status=0} ; END {exit status}' ; then
			...
			echo -e
			${monobin} ${gameopt} ${mcgdir}/MCGalaxy.exe
			if $autorestart ; then
				$0
			else
				exit 0
